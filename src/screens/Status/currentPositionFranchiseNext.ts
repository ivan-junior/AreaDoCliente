...
export function currentPositionFranchiseNext(data: IStatus) {
    const status = {
        position: 1,
        labels: {
    ...
    if(isNullEmptyOrUndefined(record.Gest_o_CST__r)){
        return status
    } else {
        if (isNullEmptyOrUndefined(record.Gest_o_CST__r?.Data_Envio_Autoriza_o_Subcontrata_o__c)) {
            return status
        } else {
            status.position = 24
        }
    }
    if (isNullEmptyOrUndefined(record.Gest_o_CST__r?.Data_Agendamento_Instala_o__c)) {
        return status
    } else {
        status.position = 25
        status.labels.Data_Agendamento_Instala_o__c = `Agendada para: ${formatDateToLocalTimezone(record.Gest_o_CST__r?.Data_Agendamento_Instala_o__c)}`
    }
    if (isNullEmptyOrUndefined(record.Data_em_Instala_o_Acompanhamento__c)) {
        return status
    } else {
        status.position = 26
    }
    if (isNullEmptyOrUndefined(record.Fim_da_Instala_o__c)) {
        return status
    } else {
        status.position = 28
        status.labels.Fim_da_Instala_o__c = `Concluída em: ${formatDateToLocalTimezone(record.Fim_da_Instala_o__c)}`
    }
