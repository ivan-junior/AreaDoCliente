...
export const StepIndicatorLabelFranchiseNext = ({configText, infoLabel}: any) => {
    const headerPositions = [0,5,8,12,16,18,20,22,27,31,34,36]
...
function renderConfigSubLabel(stepPosition: number, infoLabel: any) {
    ...
    } else if (stepPosition == 24 && !isNullEmptyOrUndefined(infoLabel.Data_Agendamento_Instala_o__c)) {
        return (
            <Text style={styles.stepSubLabel}>
                {infoLabel.Data_Agendamento_Instala_o__c}
            </Text>
        )
    } else if (stepPosition == 26 && !isNullEmptyOrUndefined(infoLabel.Fim_da_Instala_o__c)) {
        return (
            <Text style={styles.stepSubLabel}>
                {infoLabel.Fim_da_Instala_o__c}
            </Text>
        )
    } else if (stepPosition == 28 && !isNullEmptyOrUndefined(infoLabel.Data_Recebimento_RIC__c)) {
        return (
            <Text style={styles.stepSubLabel}>
                {infoLabel.Data_Recebimento_RIC__c}
            </Text>
        )
    } else if (stepPosition == 29 && !isNullEmptyOrUndefined(infoLabel.Data_In_cio_An_lise_RIC__c)) {
        return (
            <Text style={styles.stepSubLabel}>
                {infoLabel.Data_In_cio_An_lise_RIC__c}
            </Text>
        )
    } else if (stepPosition == 30 && !isNullEmptyOrUndefined(infoLabel.Data_do_RIC_Aprovado__c)) {
        return (
            <Text style={styles.stepSubLabel}>
                {infoLabel.Data_do_RIC_Aprovado__c}
            </Text>
        )
    } else if (stepPosition == 32 && !isNullEmptyOrUndefined(infoLabel.Data_Solicita_o_da_Vistoria_Distrib__c)) {
        return (
            <Text style={styles.stepSubLabel}>
                {`${infoLabel.Data_Solicita_o_da_Vistoria_Distrib__c}`}
            </Text>
        )
    } else if (stepPosition == 33 && !isNullEmptyOrUndefined(infoLabel.Data_de_Aprova_o_da_Vistoria__c)) {
        return (
            <Text style={styles.stepSubLabel}>
                {`${infoLabel.Data_de_Aprova_o_da_Vistoria__c}`}
            </Text>
        )
    } else if (stepPosition == 35 && !isNullEmptyOrUndefined(infoLabel.Data_Medidor_Trocado__c)) {
        return (
            <Text style={styles.stepSubLabel}>
